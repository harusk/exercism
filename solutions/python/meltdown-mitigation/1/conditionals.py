...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if percentage_value >= 80.0:
        return 'green'
    elif percentage_value < 80.0 and percentage_value >= 60.0:
        return 'orange'
    elif percentage_value < 60.0 and percentage_value >= 30.0:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if criticality < (0.90 * threshold):
        status_code = 'LOW'
    elif(criticality >= (0.90 * threshold) and
         criticality <= (1.10 * threshold)):
        status_code = 'NORMAL'
    else:
        status_code = 'DANGER'
    return status_code
